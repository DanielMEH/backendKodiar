...
app.get("/inventario", (req, res) => {
  ...
  if (session.userid) {
      ...
      (error, rows) => {
        if (error) {
          return res
            .status(501)
            .send(
              "<script>alert('Hubo un error'); window.location = '/'</script>"
            );
        } else {
    ...
    return res
      .status(501)
      .send(
        "<script>alert('No hay una cuenta iniciada'); window.location = '/login'</script>"
      );
  }
});
...
app.get("/buscador", (req, res) => {
  ...
  if (session.userid) {
    db.all(
      "SELECT * FROM producto WHERE idUsuario=$user",
      {
        $user: session.userid,
      },
      (err, rows) => {
...
app.get("/producto", (req, res) => {
  ...
  if (session.userid) {
    db.all(
      `SELECT * FROM  categoria  WHERE idusuario =$email`,
      {
      ...
      (error, rows) => {
        if (error) {
          ...
          return res
            .status(501)
            .send(
              "<script>alert('Hubo un error al cargar los datos'); window.location = '/'</script>"
            );
        }
      ...
      if (!error) {
        ...
        if (rowUnidades < unidad) {
          return res
            .status(500)
            .send(
              "<script>alert('No hay suficienes unidades'); window.location = '/dasboard'</script>"
            );
        }
        ...
        db.run(
          `UPDATE producto SET unidades = ?  WHERE id = ?`,
          [restarUnidad, req.body.searchcode],
          (error) => {
            if (error) {
              console.log(error);
            ...
            return res
              .status(200)
              .send(
                "<script>alert('Los datos se actualizaron'); window.location = '/dasboard'</script>"
              );
          }
        );
      } else {
        return res
          .status(200)
          .send(
            "<script>alert('error en los datos'); window.location = '/dasboard'</script>"
          );
      }
    }
  );
  ...
  if (session.userid) {
    db.get(
      ...
      (error, rows) => {
        ...
        return res.render("cuenta", { userPerfil: rows });
      }
    );
  }
});
...
app.post(
  ...
  [
    ...
    body(
      "id_categoria",
      "Debes elegir en que categoria se va a guardar el producto"
    )
      .exists()
      .isLength({ min: 1 }),
  ],
  (req, res) => {
    ...
    if (!errorProductDate.isEmpty()) {
      ...
      db.run(
        `INSERT INTO producto(id,namep,unidades,
      precioc,preciov,fechaven,descripcion,id_categoria,idUsuario) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        ...
        (error, rows) => {
          if (!error) {
            return res.send(
              "<script>alert('Producto registrado exitosamente'); window.location = '/producto'</script>"
            );
          } else {
            if (error.code == "SQLITE_CONSTRAINT") {
              return res.send(
                "<script>alert('El codigo del producto ya existe'); window.location = '/producto'</script>"
              );
            }
          }
        }
...
app.post(
  ...
  [
    body(
      "nombre",
      "El nombre no es valido asegurate de que no tenga caracteres especiales y no este vacio"
    )
      .exists()
      .isLength({ min: 5 }),
    body(
      "documento",
      "El documento no es valido asegurate de que tenga mas de 8 numeros y no este vacio"
    )
      .exists()
      .isLength({ min: 8 }),
    body(
      "correo",
      "El correo no es valido asegurate de que no este vacio y este bien escrito"
    )
      .exists()
      .isEmail(),
    body(
      "password",
      "la contraseña no es valida debe tener mas de 5 caracteres"
    )
      .exists()
      .isLength({ min: 5 }),
  ],
      ...
      db.all("SELECT * FROM usuario", (error, rows) => {
        for (let i = 0; i < rows.length; i++) {
          if (rows[i].correo == correo) {
            return res.send(
              "<script>alert('Este Correo ya existe'); window.location = '/registro'</script>"
            );
          }
        }

        db.run(
          `INSERT INTO usuario(documento,nombre,correo,password) VALUES(?, ?, ?, ?)`,
          [documento, nombre, correo, hash],
          async (error, rows) => {
            if (!error) {
           ...
           <div class="text">
             ...
             <p style="text-align:center; display: block;">Kodiar es una aplicación de administración para el inventario de los productos de tu negocio , en la cual podras llevar un control permanente de todos ellos, necesidades de abastecimiento, reporte de vencimientos, rotación de productos y podras utilizarla para el manejo administrativo y financiero de tu negocio.</p>
              ...
              if (error.code == "SQLITE_CONSTRAINT") {
                return res.send(
                  "<script>alert('Este usuario ya existe'); window.location = '/registro'</script>"
                );
              }
            }
          }
...
app.post("/login", (req, res) => {
  ...
  db.get(
    ...
    (error, rows) => {
      if (error) {
        res.send(
          "<script>alert('El correo o contraseña no existe') window.location = '/login' </script>"
        );
      }
...
app.post(
  ...
  (req, res) => {
    ...
    if (!errorCategoria.isEmpty()) {
      ...
      return res.render("ErrorCategoria", {
        validacion: validacion,
        valores: valores,
      });
    } else {
      ...
      db.run(
        `INSERT INTO categoria(nombre,imagen,idusuario) VALUES (?, ?, ?)`,
        [nombre, imagen, session.userid],
        (error, rows) => {
          if (error) {
            console.log(error);
            return res.send(
              "<script>alert('La categoria no se creo vuelva a intentarlo'); window.location = '/categoria'</script>"
            );
          } else {
            res.send(
              "<script>alert('Categoria creada exitosamente'); window.location = '/categoria'</script>"
            );
          }
        }
      );
...
app.get("/delete/:id", (req, res) => {
  db.run(
    ...
    (error, rows) => {
      if (error) {
        return res.send(
          "<script>alert('La categoria no se elimino vuelva a intentarlo'); window.location = '/inventario'</script>"
        );
      } else {
        res.send(
          "<script>alert('Producto eliminado exitosamente'); window.location = '/inventario'</script>"
        );
      }
    }
  );
...
app.get("/editProduct/:id", (req, res) => {
  ...
  db.get("SELECT * FROM producto WHERE id = ?", [id], (error, rows) => {
    if (!error) {
      ...
      return res.send(
        "<script>alert('Error vuelva a intentarlo'); window.location = '/inventario'</script>"
      );
    }
  });
});
...
app.post("/update/:id", (req, res) => {
  ...
  db.run(
    `UPDATE producto SET namep = ?, unidades = ?, precioc = ?, preciov = ?, fechaven = ?,
   descripcion = ? WHERE id = ?`,
    ...
    (error, rows) => {
      if (!error) {
        res.send(
          "<script>alert('Producto actualizado exitosamente'); window.location = '/inventario'</script>"
        );
      } else {
        console.log(error);
        return res.send(
          "<script>alert('El producto no se actualizo'); window.location = '/inventario'</script>"
        );
      }
    }
  );
